...
int main(int argc, char **argv)
{
	...
	if (families != NULL) free(families);
...
int parse_script(char *filename, script_ent_t *script)
{
	...
	if (cfg_parse(pcfg, filename) == CFG_PARSE_ERROR) return -1;
	...
	for (i=0; i<(script->script_length << 1); i++) script->script[i] = (uint8_t)cfg_getnint(pcfg, "script", i);
...
uint16_t get_script_by_name(char *name)
{
	...
	if (strcmp(name, "NO_SCRIPT") == 0) return 0;
	for (ret = 0; ret < num_rd_scripts; ret++) {
		if (strcmp(name, scripts[ret].name) == 0) return ret+1;
	}
...
uint16_t get_family_by_name(char *name)
{
	...
	for (ret = 0; ret < num_rd_families; ret++) {
		if (strcmp(name, families[ret].name) == 0) return ret;
	}
...
int parse_device(char *filename, device_ent_t *device)
{
	...
	if (cfg_size(pcfg, "config_blank") != 8) printf("WARNING : size of \"config_blank\" array is wrong!\n");
	for (i=0; i<8; i++)
...
char *script_name(uint16_t script_id)
{
	...
	if (script_id == 0) return "NO_SCRIPT";
...
int read_device_file(char *filename)
{
	...
	for (i=0; i<hdr.num_scripts; i++) {
		...
		for (j=0; j<i; j++) if (strcmp(scripts[j].name, scripts[i].name) == 0) n++;
		if (n > 0) {
...
int build_device_file(char *dir, char *outfile)
{
	...
	if (cfg_parse(pcfg, fname) == CFG_PARSE_ERROR) return -1;
	...
	printf("reading scripts..."); fflush(stdout);
	if (verbose > 0) printf("\n");
	for (i=0; i<num_rd_scripts; i++) {
		if (verbose > 0) printf("reading script file \"%s\"\n", cfg_getnstr(pcfg, "scripts", i));
	...
	printf("reading families..."); fflush(stdout);
	if (verbose > 0) printf("\n");
	for (i=0; i<num_rd_families; i++) {
		if (verbose > 0) printf("reading family file \"%s\"\n", cfg_getnstr(pcfg, "families", i));
	...
	printf("reading devices..."); fflush(stdout);
	if (verbose > 0) printf("\n");
	for (i=0; i<num_rd_devices; i++) {
		if (verbose > 0) printf("reading device file \"%s\"\n", cfg_getnstr(pcfg, "devices", i));
	...
	// write all families
	for (i=0; i<num_rd_families; i++) {
		if (verbose > 0) printf("writing \"%s\" to file\n", families[i].name);
	...
	fprintf(stdout, "start of scripts 0x%x\n", ftell(fout));
	...
	for (i=0; i<num_rd_scripts; i++) {
		...
		else write_str(fout, scripts[i].name);
...
void dumphex(uint8_t *buff, int length)
{
	...
	for (i=0; i<length; i++) {
		...
		if ((i & 15) == 15) {
			printf(" - ");
			for (j=(i-15); j<i; j++) {
				if ((buff[j] > 0x20) && (buff[j] < 0x7f)) printf("%c", buff[j]);
				else printf(".");
			}
		}
	}
